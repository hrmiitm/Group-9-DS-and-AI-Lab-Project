"""
tools/tool_job_boards.py
Check if a job posting appears on trusted job boards.
100% free — DuckDuckGo search, no API key.

Verdict logic:
  3+ boards → strong_presence   (likely legitimate)
  1-2 boards → moderate_presence
  0 boards   → not_found_on_boards  (red flag)
"""
import time
...
def check_job_boards(job_title: str, company_name: str, location: str | None = None) -> dict:
    """
    ...
    return {
        ...
        "data": {
            ...
            "verdict": (
                "strong_presence"    if found_count >= 3 else
