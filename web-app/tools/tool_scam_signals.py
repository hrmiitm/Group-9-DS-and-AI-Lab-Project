"""
tools/tool_scam_signals.py
Scam signal detector — keyword-based weighted scoring.
100% free — pure Python keyword matching, no API, no library.
...
RULES = {
    "asks_for_money": {
        "keywords": [
            "registration fee", "pay a fee", "processing fee",
    ...
    "pre_interview_docs": {
        "keywords": [
            "aadhaar copy", "pan card copy", "passport copy",
    ...
    "vague_company": {
        "keywords": [
            "undisclosed company", "confidential client",
...
def detect_scam_signals(job_text: str) -> dict:
    ...
    OUTPUT: {ok, data: {scam_score, risk_level, signals_found, signals_count, matched_signals}}
    """
    if not job_text:
    ...
    return {
        ...
        "data": {
            ...
            "signals_found":   list(matched.keys()),
